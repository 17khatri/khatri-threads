...
import {
  createPasswordResetCookie,
  passwordResetCookie,
} from "@/lib/password-reset";
import { prisma } from "@/lib/prisma";
import { forgotPasswordOtpSchema } from "@/lib/validators/forgot-password";
...
export async function POST(request: Request) {
  const parsed = forgotPasswordOtpSchema.safeParse(
  ...
  try {
    ...
    if (!user || !emailOtp || emailOtp.expiresAt.getTime() <= Date.now()) {
      if (emailOtp) {
    ...
    response.cookies.set(
      ...
      createPasswordResetCookie(parsed.data.email),
      {
        httpOnly: true,
        sameSite: "lax",
        secure: process.env.NODE_ENV === "production",
        path: "/",
        maxAge: passwordResetCookie.maxAge,
      },
    );
